...
def authorize():
    ...
    if db.is_user_in_db(user_info['email']):
        ...
        return redirect('/logout?msg=1')


@app.route('/logout', methods = ['GET','POST'])
def logout_user():
    global user
    if dict(session).get('email', None) is not None:
        db.manage_activity_results(db.get_id_attivita(user), user)    
    ...
    user = ''
    print(user)
    #client.disconnect() #stoppo la ricezione dei messaggi (per il momento quando esco)
    return redirect('/') if request.args.get('msg') is None else redirect('/?msg=1')
